...
import { detectDirection, farsiRatio, isFarsiPredominant, type Direction } from 'rtl-core';
import { z } from 'zod';
...
function recommend(direction: Direction): string {
  switch (direction) {
    case 'rtl':
 ...
 */
export function detectFarsi(text: string): DetectFarsiResult {
...
export function registerDetectFarsi(server: McpServer): void {
  server.registerTool(
    'detect_farsi',
    {
